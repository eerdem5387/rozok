...
const SCHOOL_NAME = "RİZE - GÜNEYSU - ÖZEL GÜNEYSU OKULLARI";
...
export async function POST(request: Request) {
  try {
    ...
    let exists = await prisma.guneysuBasvuru.findUnique({ where: { refNo } });
    while (exists) {
      refNo = generateRefNo();
      exists = await prisma.guneysuBasvuru.findUnique({ where: { refNo } });
    }

    await prisma.guneysuBasvuru.create({
      data: {
        refNo,
    ...
    console.error("[api/guneysu-basvuru] POST error:", e);
    return NextResponse.json(
